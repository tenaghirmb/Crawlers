...
def get_comment_page(bookid,pn):
    try:
        ...
        print('获取'+bookid+'评论第'+str(pn)+'页出错！\t'+time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))
...
def get_page_number(cp):
    if(cp):
        ...
        if data and 'data' in data.keys():
            ...
            print('获取评论页数失败！\t'+time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))
            #logger.warning('获取评论页数失败！')
            return None
    else:
        print('评论首页加载不成功！\t'+time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))
...
def parse_comment_page(cp):
    if(cp):
        ...
        if data and 'data' in data.keys():
            ...
            print('解析评论页面失败！\t'+time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))
...
def main():
    ...
    for i in range(a,b):
        ...
        print(str(i)+'-完成\t'+time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))
